...
2. AIHub "쉐이프리스 의류 및 포즈 데이터" 기반 fit analyzer / keypoint / segmentation / confidence scoring 기준 설계
3. 공개 데이터셋 후보 검토
4. 필요한 경우 VITON-HD 또는 DressCode 일부 샘플 검토
5. FIT 데이터셋 기반 fit-aware scoring은 후속 확장 방향으로 검토
...
- pseudo target: StableVITON, IDM-VTON, CatVTON 같은 VTON 모델이 생성한 virtual try-on 결과 이미지
...
- 원본 데이터셋 이미지
- AIHub 원본 이미지, annotation, JSON 파일
